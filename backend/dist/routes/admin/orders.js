...
const router = Router();
// -------- util -----------
const VALID_STATUS = ["pending", "paid", "shipped", "canceled"];
/* ============================================================
   GET /admin/orders
...
router.get("/", async (req, res) => {
    ...
    try {
        ...
        const total = (await db.query(totalSQL, params)).rows[0]?.count;
        /* lista */
...
router.put("/:id/status", async (req, res) => {
    ...
    try {
        const { rows: [order], } = await db.query(`UPDATE orders SET status = $1, updated_at = NOW()
       WHERE id = $2
       RETURNING *`, [status, id]);
        return order
            ? res.json(order)
