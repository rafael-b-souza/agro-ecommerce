...
router.get("/", async (req, res) => {
    ...
    const limitNum = Math.min(Math.max(parseInt(limit, 10) || 10, 1), 50); // cap de 50 p/ evitar abusos
    const offset = (pageNum - 1) * limitNum;
    /* ----- cláusula dinâmica de filtro ----- */
    let where = "";
    const params = [];
    if (category) {
    ...
    try {
        /* 1) Total de registros — para meta */
        const totalSQL = `
      SELECT COUNT(*)::int AS count
      FROM products
      LEFT JOIN categories ON categories.id = products.category_id
      ${where}`;
        const { rows: [{ count: total }], } = await db.query(totalSQL, params);
        /* 2) Consulta paginada */
